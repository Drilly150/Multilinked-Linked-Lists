...
namespace ProyectoPert.Servicios
{
    ...
    public class GestorProyecto
    {
        ...
        public void MostrarDiagramaTexto()
        {
            ...
            var listaTareas = _listaTareas;
            if (listaTareas.GetInicio() == null)
            {
                Console.WriteLine("No hay tareas para mostrar en el diagrama.");
                return;
            }

            // Usaremos una cola para el recorrido por niveles y un set para rastrear nodos ya procesados.
            ...
            {
                if (nodoActual.Dato.Predecesores.GetInicio() == null)
                {
                    cola.Enqueue(nodoActual.Dato);
                    nodosVisitados.Add(nodoActual.Dato.Id);
                }
            ...
            // 2. Procesar la cola nivel por nivel.
            while (cola.Count > 0)
            {
                int nivelSize = cola.Count;
                var sbNivel = new StringBuilder();
                
                for (int i = 0; i < nivelSize; i++)
                {
                    var tareaActual = cola.Dequeue();
                    
                    // Imprimir la tarea actual en el formato deseado.
                    sbNivel.AppendFormat("( {0}: {1:F1} días )  ", tareaActual.Id, tareaActual.TiempoEsperado);

                    // Revisar sus sucesores.
                    var sucesorNode = tareaActual.Sucesores.GetInicio();
                    while (sucesorNode != null)
                    {
                        var tareaSucesora = listaTareas.BuscarPorId(sucesorNode.IdTarea);
                        if (tareaSucesora != null && !nodosVisitados.Contains(tareaSucesora.Id))
                        {
                            // Un sucesor solo se agrega a la cola si TODOS sus predecesores ya han sido visitados.
                            bool todosPredecesoresVisitados = true;
                            var predecesorDeSucesorNode = tareaSucesora.Predecesores.GetInicio();
                            while (predecesorDeSucesorNode != null)
                            {
                                if (!nodosVisitados.Contains(predecesorDeSucesorNode.IdTarea))
                                {
                                    todosPredecesoresVisitados = false;
                                    break;
                                }
                                predecesorDeSucesorNode = predecesorDeSucesorNode.Siguiente;
                            }

                            if (todosPredecesoresVisitados)
                            {
                                cola.Enqueue(tareaSucesora);
                                nodosVisitados.Add(tareaSucesora.Id);
                            }
                        }
                        sucesorNode = sucesorNode.Siguiente;
                    }
                }
                Console.WriteLine(sbNivel.ToString());
            }
